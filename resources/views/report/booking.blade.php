                ...
                <div class="card-header d-flex justify-content-between">
                    ...
                    <form action="{{ route('admin.report.booking.cari') }}" method="GET">
                        ...
                        <div class="d-flex justify-content-end mx-2">
                            ...
                            <input type="date" class="form-control @error('endDate') is-invalid @enderror mr-2" name="endDate" id="endDate" required>
                            @error('endDate')
                                <div class="invalid-feedback">
                                    <strong>{{ $message }}</strong>
                                </div>
                            @enderror
                            <button type="submit" class="btn btn-secondary btn-sm"><i class="fa fa-search"></i> Cari</button>      
                        </div> 
                    </form> 
                </div>
                @if(request('startDate') && request('endDate'))
                    <span class="badge badge-light mx-5 mt-3"><h6 class="p-1">Laporan From : {{ $startDate }} &middot; To : {{ $endDate }}</h6></span></th>
                @endif
                <div class="card-body">
                        ...
                        <table class="table table-hover">
                            ...
                            @forelse ($bookings as $booking)
                                <tbody>
                                    ...
                                    <td>
                                        <span class="badge badge-light">{{ Str::upper($booking->room->name . " (" . $booking->room->category->name . ")") }}<span>
                                    </td>
                                    <td>
                                        <span class="badge badge-light">{{ Str::upper($booking->customer->FullName) }}<span>
                                    </td>
                                    <td>
